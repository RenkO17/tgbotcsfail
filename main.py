...
def parse(number_game):
    ...
    while True:
        ...
        if result != '{"error_code":-1,"error_text":"Game not found"}':
            ...
            result_message = color_for_mess[0] + ' K=' + str(coef).replace('.', '_') + ' ' + time_for_mess + ' #{}'.format(color_for_mess[1]) 
            result_message += '\n#K{} #T{} #D{}'.format(str(coef).replace(".", "_"), time_for_mess[:time_for_mess.find(" ")].replace(":", ""), time_for_mess[time_for_mess.find(" ") + 1:].replace(".", "_"))
